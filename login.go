...
func main() {
	...
	if bExtractTokenValueOK {
		gLogger.Info("======步骤3：获取验证码 ======")
		postD := map[string]string{}
		postD["token"] = strLoginToken
		postD["tpl"] = "mn"
		postD["apiver"] = "v3"
		postD["tt"] = strconv.FormatInt(time.Now().Unix(),10)
		postD["fr"] = "login"
		postD["vcodetype"] = "d1567lt49kV/xXr3X6TPFYzsHKU9pjx+jpu70VtHX6BWrty0QM4D7CXc7nFM4e3lCH3fL4X1IrteNalnAgak1hJM5THLnuUrMRsB"
		baiduReggetcodestr := "https://passport.baidu.com/v2/?reggetcodestr"
		var require_str string
		for n,v := range postD {
			require_str += "&"+n+"="+v
		}
		baiduReggetcodestr += require_str
		fmt.Println(baiduReggetcodestr)
		getbaiduReggetcodestrRespHtml := getUrlRespHtml(baiduReggetcodestr, nil)
		gLogger.Debug("getbaiduReggetcodestrRespHtml=%s", getbaiduReggetcodestrRespHtml)
		dbgPrintCurCookies()
		verifyStr := gjson.Get(getbaiduReggetcodestrRespHtml,"data.verifyStr")
		fmt.Println(verifyStr)
		baiduGetVCodeUrl := "https://passport.baidu.com/cgi-bin/genimage?"+verifyStr.String()
		fmt.Println(baiduGetVCodeUrl)
	}

	//step3: verify returned cookies
	bLoginBaiduOk := false
	if bGotCookieBaiduid && bExtractTokenValueOK {
		gLogger.Info("======步骤4：登陆百度并检验返回的Cookie ======")
		...
		gLogger.Info("Baidu Verifycode:")
		_,err3 := fmt.Scanln(&strBaiduVerifycode)
		if nil == err3 {
			gLogger.Debug("strBaiduVerifycode=%s", strBaiduVerifycode)
		}

		postDict["username"] = strBaiduUsername
		postDict["password"] = strBaiduPassword
		postDict["verifycode"] = strBaiduVerifycode
